...
podTemplate(label: label, containers: [
    containerTemplate(name: 'dynamodb',
                      ...
                      command: 'java -jar /home/dynamodblocal/DynamoDBLocal.jar -inMemory -sharedDb -port 8007',
                      ports: [portMapping(name: 'dynamoport', containerPort: 8007, hostPort: 8007)]),
    ...
    node(label) {
        ...
        container('node'){
            ...
            stage ("create-seed-table") {
                ...
                --table-name cvs-local-test-stations \
                ...
                --endpoint-url http://localhost:8007
                '''

                sh "sls dynamodb seed --seed=test-stations"
            }
            stage ("sonar") {
            ...
            stage("upload to s3") {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding',
                 ...
                 sh "aws s3 cp ${LBRANCH}.zip s3://cvs-services/test-stations/${LBRANCH}.zip --metadata sha256sum=\"\$(openssl dgst -sha256 -binary ${LBRANCH}.zip | openssl enc -base64)\""
                }
            }
        }
    }
